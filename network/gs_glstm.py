...
class GsGLstm(nn.Module):
    ...
    def __init__(self, options):
        ...
        self.w_in_i_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_in_i_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.w_out_i_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_out_i_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.b_i_gate = nn.Parameter(torch.ones(self.g_hidden_dim))

        self.w_in_o_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_in_o_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.w_out_o_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_out_o_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.b_o_gate = nn.Parameter(torch.ones(self.g_hidden_dim))

        self.w_in_f_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_in_f_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.w_out_f_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_out_f_gate = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.b_f_gate = nn.Parameter(torch.ones(self.g_hidden_dim))

        self.w_in_cell = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_in_cell = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.w_out_cell = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.u_out_cell = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.g_hidden_dim, self.g_hidden_dim)))
        self.b_cell = nn.Parameter(torch.ones(self.g_hidden_dim))
        ...
        self.node_cell = nn.init.xavier_uniform_(torch.Tensor(self.batch, self.max_node_num, self.g_hidden_dim))

        self.rep = torch.empty(self.batch, self.max_node_num, self.max_in_node_num, self.g_hidden_dim)
        self.rep = nn.Parameter(nn.init.xavier_uniform_(self.rep))
    ...
    def forward(self, node_num, lemmas, lemmas_idx, lemmas_chars, in_nodes, in_labels, out_nodes, out_labels, entity_indexs, \
        ...
        # word_rep = self.mask_select_of_node(word_rep, node_mask)
        self.node_hidden = word_rep
        self.graph_hidden = word_rep
        # get graph_representation
        graph_rep, graph_cell = self._get_graph_rep(in_nodes, in_node_mask, out_nodes, out_node_mask,
                                                    in_node_rep, out_node_rep)
        # res = word_rep.mean(-2)
        # res = graph_rep
        # res = res.view((8, -1, 150))
        # res = res.mean(-2)
        # res = self.classifier(res)
        # res = Func.softmax(res, dim=-1)
    ...
    def _get_graph_rep(self, in_nodes, in_node_mask, out_nodes, out_node_mask, in_node_rep, out_node_rep):
        ...
        graph_rep = torch.zeros(self.layer_num, self.batch, self.max_node_num, self.g_hidden_dim)
        ...
        for layer_idx in range(self.layer_num):
            ...
            edge_i_gate = torch.sigmoid(torch.matmul(in_node_rep, self.w_in_i_gate)
                                       ...
                                       + self.b_i_gate)
            edge_o_gate = torch.sigmoid(torch.matmul(in_node_rep, self.w_in_o_gate)
                                       ...
                                       + self.b_o_gate)
            edge_f_gate = torch.sigmoid(torch.matmul(in_node_rep, self.w_in_f_gate)
                                       + torch.matmul(prev_in_edge_hidden, self.u_in_f_gate)
                                       + torch.matmul(out_node_rep, self.w_out_o_gate)
                                       + torch.matmul(prev_out_edge_hidden, self.u_out_f_gate)
                                       + self.b_f_gate)
            edge_cell_input = torch.sigmoid(torch.matmul(in_node_rep, self.w_in_cell)
            ...
            node_cell = self.mask_select_of_node(edge_cell, self.node_mask)
            node_hidden = self.mask_select_of_node(edge_hidden, self.node_mask)

            graph_rep[layer_idx] = node_hidden

        self.graph_rep = graph_rep
        self.graph_hidden = node_cell
    ...
    def _get_entity_rep(self, encoder_state, entity_indexs):
        ...
        rep = rep.mean(-2)
        rep = rep.view((self.batch, -1))
        rep = Func.softmax(torch.tanh(torch.matmul(rep, self.w_rel) + self.b_rel + 1e-5))
    ...
    def _get_entity_by_index(self, encoder_state, entity_indexs):
        ...
        res = nn.init.xavier_uniform_(res)
    ...
    def mask_select_of_neighbour(self, edge_hidden, mask_t, vec_dim=0):
        ...
        zero_n = torch.zeros(self.batch, len(edge_hidden[0]), neigh_len, vec_dim)
        edge_hidden = torch.cat((edge_hidden, zero_n), dim=-2)
        # print("edge_hidden1: ", edge_hidden.shape)
        zero_n = torch.zeros(self.batch, seq_len, self.max_in_node_num, vec_dim)
        edge_hidden = torch.cat((edge_hidden, zero_n), dim=1)
        # print("edge_hidden2: ", edge_hidden.shape)

        return edge_hidden

    def mask_select_of_node(self, node_hidden, mask_t, vec_dim=0):
        if vec_dim == 0:
            vec_dim = self.g_hidden_dim
        seq_len = self.max_node_num - len(node_hidden[0])
        zero_t = torch.zeros(self.batch, seq_len, vec_dim)
        node_hidden = torch.cat((node_hidden, zero_t), dim=-2)
